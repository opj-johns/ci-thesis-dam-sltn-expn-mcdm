    %% Define continuous decision variables: products A and B
    x = optimvar('x', 2, 'LowerBound', 0, 'Type','integer');
    
    % Define deviation variables (positive and negative)
    n = optimvar('n', 4, 'LowerBound', 0);  % n1 to n4 (negative deviations)
    p = optimvar('p', 4, 'LowerBound', 0);  % p1 to p4 (positive deviations)
    
    % Define the optimization problem
    prob = optimproblem;
    
    % Objective function
    prob.Objective = (1/120)*p(1) + (1/7000)*n(2) + (1/40)*n(3) + (1/40)*n(4);
    
    % Constraints
    prob.Constraints.cons1 = 4*x(1) + 3*x(2) + n(1) - p(1) == 120;      % Labour
    prob.Constraints.cons2 = 100*x(1) + 150*x(2) + n(2) - p(2) == 7000; % Profit
    prob.Constraints.cons3 = x(1) + n(3) - p(3) == 40;                  % Product A availability
    prob.Constraints.cons4 = x(2) + n(4) - p(4) == 40;                  % Product B availability
...
%% === Decision variables ===
x = optimvar('x', 2, 'LowerBound', 0);          % x1 and x2: Product A and B
n = optimvar('n', 4, 'LowerBound', 0);          % n1 to n4: Negative deviations
p = optimvar('p', 4, 'LowerBound', 0);          % p1 to p4: Positive deviations
...
% === Constraints ===
prob.Constraints.cons1 = 4*x(1) + 3*x(2) + n(1) - p(1) == 120;
prob.Constraints.cons2 = 100*x(1) + 150*x(2) + n(2) - p(2) == 7000;
prob.Constraints.cons3 = x(1) + n(3) - p(3) == 40;
prob.Constraints.cons4 = x(2) + n(4) - p(4) == 40;

% === Convert to solver-based form ===
problem = prob2struct(prob);

% === Solve using intlinprog (no integer vars yet, but syntax is same) ===
...
%% Dam selection::Weighted Goal Programming
% === Decision variables ===
...
% === Decision variables ===
x = optimvar("x",2,"LowerBound",0);
n = optimvar("n",4,"LowerBound",0);
p = optimvar("p",4,"LowerBound",0);
...
prob.Constraints.cons3 = x(1) + n(3) - p(3) == 40;
prob.Constraints.cons4 = x(2) + n(4) - p(4) == 40;
prob.Constraints.cons5 = (p(1)/120) <= D(1);
prob.Constraints.cons6 = (n(2)/7000) <= D(1);
prob.Constraints.cons7 = (n(3)/40) <= D(1);
prob.Constraints.cons8 = (n(4)/40) <= D(1);
...
%% Dam selection:: Chebyshev Goal Programming
...
[sol,fval,exitflag,output] = intlinprog(problem);
    ...
    % === Objective Function ===
    alpha = 0.8;
    prob.Objective = alpha*D(1) + (1 - alpha)*((1/120)*p(1) + (1/7000)*n(2) + (1/40)*n(3) + (1/40)*n(4));
...
%% Dam selection:: Extended Goal Programming
...
% === Solve using intlinprog ===
[sol,fval,exitflag,output] = intlinprog(problem);
%% Dam site selection:: Lexicographic GP - Priority 1
...
priority_1_objective = fval;
priority_1_sol = sol;

%% Dam site selection:: Lexicographic GP - Priority 2
...
prob.Constraints.cons13 = (1/47)*n(1) + (1/3)*n(2) + (1/0.52)*n(5) + (1/0.68)*n(10) == priority_1_objective;
...
priority_2_objective =  fval;
priority_2_sol = sol;

%% Dam site selection:: Lexicographic GP - Priority 3
...
prob.Constraints.cons14 = (1/0.04)*p(3) + (1/48.74)*p(4) + (1/22.07)*n(6) == priority_2_objective;
